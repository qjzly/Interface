  ...
  local function applyBackground(bu)
    ...
    if cfg.background.showbg or cfg.background.showshadow then
      ...
      bu.bg:SetPoint("TOPLEFT", bu, "TOPLEFT", -3, 3)  --  阴影边框，此数值需要与inset相同，inset向里的边界 才能与 背景边框 重合，数值就为边框宽度
  ...
  local function styleActionButton(bu)
    ...
    ic:SetPoint("TOPLEFT", bu, "TOPLEFT", 2, -2)  --图标与背景边框之间的空隙
  ...
  local function stylePetButton(bu)
	...
	local ho  = _G[name.."HotKey"]  --定义宠物动作条快捷键
	ho:Hide()  --隐藏宠物动作条快捷键
    nt:SetAllPoints(bu)
    --applying color
    nt:SetVertexColor(cfg.color.normal.r,cfg.color.normal.g,cfg.color.normal.b,1)
