...
function PositionButtonCollector(self, screenQuadrant)
    MBCF:ClearAllPoints()
    ...
    if BaudErrorFrameMinimapButton then
        ...
        BaudErrorFrameMinimapButton.ClearAllPoints = dummy
        BaudErrorFrameMinimapButton.SetPoint = dummy
    end
    prevLineButton = nil
    prevButton = nil
    positioned = 0
        ...
        if not buttons[i].hooked then
            hooksecurefunc(buttons[i], "SetPoint", function(self, _, anchor)
                    if anchor == Minimap or type(anchor) == "number" then
                        PositionButtonCollector(Minimap)
                    end
                end)
            buttons[i]:HookScript("OnShow", function() PositionButtonCollector(Minimap) end)
            buttons[i]:HookScript("OnHide", function() PositionButtonCollector(Minimap) end)
            buttons[i].hooked = true
        end
    end
    local line = math.ceil(Minimap:GetWidth() / 20)
    local rows = math.floor(positioned / line) + 1
