...
local function SkinSkada()
	...
	barmod.ApplySettings = function(self, win)
		...
		skada:SetTexture([[Interface\AddOns\!aMini\media\statusbar.tga]])
		skada:SetSpacing(barSpacing)
		skada:SetFrameLevel(5)

		local titlefont = CreateFont("TitleFont"..win.db.name)
		titlefont:SetFont([[Fonts\ARKai_T.ttf]], 12, "OUTLINE")
		win.bargroup.button:SetNormalFontObject(titlefont)

		local color = win.db.title.color
		win.bargroup.button:SetBackdropColor(unpack(0, 0, 0))
		...
		for _,window in ipairs(self:GetWindows()) do
			for i,v in pairs(window.bargroup:GetBars()) do
				if not v.BarStyled then
					...
					v.label:SetFont([[Fonts\ARKai_T.ttf]], 12, "OUTLINE")
					--v.label.SetFont = R.dummy
					v.timerLabel:SetFont([[Fonts\ARKai_T.ttf]], 12, "OUTLINE")
