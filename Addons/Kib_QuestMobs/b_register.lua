...
--<<REGISTER THE ADDON WITH KIB_CONFIG (If available)>>---------------------------------------------<<>>
    ...
    local SavedVars, GlobalDatabase = Defaults, {Font = "Fonts\\FRIZQT__.TTF", Localization = GetLocale()}

    -- Access Kib_Config, which returns valuable data
    if Kib_Register_Addon then
        SavedVars, GlobalDatabase = Kib_Register_Addon("Quest Mobs", 1.12, Defaults) -- (Addon_Name, Addon_Version, Defaults)
    end
    ...
    AddonDB[1] = {["addonName"] = ThisAddon}                                        -- Local Database
