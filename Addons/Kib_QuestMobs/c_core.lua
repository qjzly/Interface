...
--<<HANDLE QUEST DATA>>-----------------------------------------------------------------------------<<>>
    ...
    local function HandleQuestData(plateData)                                                   --GlobalDatabase.EventBucket_AddLine("Kib_QuestMobs", "Plate Updated: " .. plateData.unitName)
        ...
        if QuestData.QuestType > 0 then
            ...
            local Colour = QuestData.QuestType == 1 and SavedVars.AreaQuestcolor or SavedVars.NormalQuestcolor
...
--<<SCAN QUEST DATA>>-------------------------------------------------------------------------------<<>>
    ...
    local function ScanPlate(plateData)                                                         --GlobalDatabase.EventBucket_AddLine("Kib_QuestMobs", "Plate Scanned and Cached: " .. plateData.unitName)
        local plateFrame, QuestType, QuestNoObjectives, QuestTasks = plateData.unitFrame, 0, nil, {}
        ...
        for i = 3, Tooltip:NumLines() do
            ...
            if text_b == 0 and text_r > 0.99 and text_g > 0.82 then 
                QuestNoObjectives = true
            else
                local Unit_Name, Progress = string.match(text, "^ ([^ ]-) ?%- (.+)$")

                if Unit_Name and (Unit_Name == "" or Unit_Name == LocalDatabase.Player_Name) then
                    if Progress then
                        ...
                        QuestNoObjectives = nil 
                    end
                end
            end
        end

        if QuestNoObjectives then QuestType = 1 end 
...
--<<HANDLE EVENTS>>---------------------------------------------------------------------------------<<>>
    ...
    function LocalDatabase.QUEST_LOG_UPDATE() wipe(CachedMobs) ScanAllPlates() end

    function LocalDatabase.QUEST_TURNED_IN() wipe(CachedMobs) ScanAllPlates() end
