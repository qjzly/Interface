...
function rematch:InitializeShare()
	...
	incoming.header.text:SetText(L["Incoming Rematch Team"])
	incoming.warning:SetText(L["A team with this name already exists.\nEdit the name or hit Save to overwrite it."])
	incoming.allClear:SetText(L["Do you want to save this team?"])
end
...
function rematch:PrepareShare(mode)
	share.header.text:SetText(mode==1 and L["Export"] or mode==2 and L["Import"] or L["Send To..."])
	...
	share.saveButton:SetText(mode==2 and SAVE or SEND_LABEL)
	if mode==1 then
		share.description.text:SetText(L["The text below describes this team in a format usable by other users. Hit Ctrl+C to copy it to the clipboard. Ctrl+V to paste it into an email, forum or elsewhere."])
	elseif mode==2 then
		share.description.text:SetText(L["If you have a team copied to the clipboard, paste it here with Ctrl+V. You can review the team above before hitting Enter or clicking Save to save the team."])
	elseif mode==3 then
		share.description.text:SetText(L["Who would you like to send this team to?"])
	end
	...
	elseif rematch.shareMode==2 then -- import
		...
		if shareString:len()==0 then
			...
			if not teamName then
				rematch:SetShareStatus(L["This is not a valid team."])
				share.saveButton:Disable()
				rematch:ClearSelected()
				return
			elseif savedTeams[teamName] then
				rematch:SetShareStatus(L["A team already has this name."])
			else
				rematch:SetShareStatus()
			end
...
function rematch:SendOnEnterPressed()
	if share.saveButton:IsEnabled() then
		...
		if rematch.shareString then
			rematch:SetShareStatus(L["Sending..."],"Interface\\Icons\\SOR-mail")
...
function rematch:SendTimeout()
	rematch:SetShareStatus(L["No response. Due to lag or no Rematch?"])
end
...
function rematch:CHAT_MSG_SYSTEM(message)
	if message==format(ERR_CHAT_PLAYER_NOT_FOUND_S,rematch.sendee) then
		rematch:SetShareStatus(L["They do not appear to be online."])
	end
end
...
--	print(prefix,message,sender)
	...
	if message=="ok" then -- sendee received team and sent back an ok
		rematch:SetShareStatus(L["The team was received!"],"Interface\\RaidFrame\\ReadyCheck-Ready")
	elseif message=="busy" then -- sendee received team but is already looking at a team
		rematch:SetShareStatus(L["They have another team waiting. Try again later."])
	elseif message=="combat" then -- sendee received team, but they're in combat
		rematch:SetShareStatus(L["They're in combat. Try again later."])
	elseif message=="block" then -- sendee received team, but has settings.DisableShare checked
		rematch:SetShareStatus(L["They have team sharing disabled."])
...
function rematch:DisplayIncoming(sender,teamName,teamTable)
	incoming.greeting:SetText("\124cffffd200"..sender..L["\124cffffffff has sent you a battle pet team!"])
