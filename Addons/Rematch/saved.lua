...
function rematch:InitializeSaved()
	...
	rematch.saveAs.header.text:SetText(L["Save Team As..."])
	rematch.saveAs.warning:SetText(L["A team already has this name."])
	rematch.saveAs.inset.enterName:SetText(L["Enter a name for this new team."])
end
...
function rematch:UpdateSavedPets()
	...
	if rematch.selectedTeam and savedTeams[rematch.selectedTeam] then
		...
		if savedTeams[rematch.selectedTeam][4] then
			rematch.saved.header.text:SetText(format("%s \124cffffffff%s",L["Saved for"],actualName))
		else
			rematch.saved.header.text:SetText(format("%s \"%s\"",L["Saved as"],actualName))
		end
		rematch:UpdateHints()
	else
		rematch.saved.header.text:SetText(L["Saved Battle Pets"])
	end
end
