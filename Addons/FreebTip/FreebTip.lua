...
local cfg = {
	...
	cursor = 0,							--Tooltip是否跟随鼠标，0不跟随，1跟随，2非战斗状态跟随。
	cursormode = 2,						--Tooltip跟随鼠标方式，1鼠标右下，2鼠标右上，3鼠标正上方。
	cursoroffset = {40, -30},			--Tooltip跟随鼠标坐标偏移值。
	hpbar = 1,							--是否显示血量条，0不显示，1显示。
	hpbarText = 4,						--是否显示血量值，0不显示，1显示当前值，2显示百分比，3显示当前/百分比，4显示当前值/最大值。
	...
	showGRank = 1,						--是否显示在公会中的阶级，0不显示，1显示。
	...
	sbHeight = 12,						--Tooltip血量/能量条高度(粗细)，不建议更改。
	backdrop = {
...
GameTooltipStatusBar:SetPoint("TOPLEFT", GameTooltipStatusBar:GetParent(), "BOTTOMLEFT", 3, -2)
GameTooltipStatusBar:SetPoint("TOPRIGHT", GameTooltipStatusBar:GetParent(), "BOTTOMRIGHT", -3, -2)
...
local function UpdateHP(self, value)
	...
	if (UnitExists(unit)) then
		...
		if (not self.text) then
			...
			self.text:SetPoint("CENTER", GameTooltipStatusBar, 0, -2)
			self.text:SetFont("Fonts\\ARKai_C.ttf", 8, "MONOCHROMEOUTLINE")
		end
...
local function style(frame)
	...
	if (not frame.ftipBD) then
		...
		ftipBD:SetBackdrop(cfg.backdrop2)
